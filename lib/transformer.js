...
var operators = transformer.operators = require('./tokens')
...
function BlockStatement(node, traverse, memory) {
  ...
  if (node.operator === operators.ASSIGN_NOT) {
  ...
  function isBlockOperator(node) {
    return node.operator === operators.ASSIGN || node.operator === operators.EQUAL
  }
...
function UnaryExpression(node, traverse) {
  ...
  switch (node.operator) {
    case operators.RELATIONAL:
      value.type = 'reference'
      value.visible = true
      break
    case operators.AMPERSAND:
    case operators.AMPERSAND_NOT:
    case operators.RELATIONAL_NOT:
      value.type = 'reference'
      value.visible = false
      break
    case operators.EXTEND:
      value.type = 'extend'
      break
